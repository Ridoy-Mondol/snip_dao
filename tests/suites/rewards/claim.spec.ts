...
import {claimRewards, lockStake, stake, updateStakeReward} from "../stakes/stake";
...
describe("REWARDS CLAIM:: 1 month with variable APY", () => {
  before(async () => {
  ...
  it("StakeA should have ~9999000000.0000 SNIPS balance", async () => {
  ...
  it("Claim reward should make all stake 0 reward", async () => {
    
    
    await claimRewards(stakerA.name.toString());
    const rows = snipstk.tables.stakes(nameToBigInt(stakerA.name)).getTableRows();
    const rewardTotal = rows.reduce((prev: number, current: Tables<'StakesTable'>): number => {
      prev += current.reward;
      return prev;
    }, 0)
    claimRewards
    expect(parseInt(rewardTotal.toString())).to.be.equal(0);
    
  });
  
  it("StakeA should now have ~9999003788.1940 SNIPS balance", async () => {
    
    const balance = getAccountBalance(xtokens, stakerA.name.toString(), '0.0000 SNIPS');
    expect(balance).to.be.equal('9999003788.1940 SNIPS');
